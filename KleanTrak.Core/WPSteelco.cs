...
namespace KleanTrak.Core
{
	public class WPSteelco : WPBase
    {
		...
		private bool m_bSteelcoFineLavaggioManuale;

		public WPSteelco(bool bSteelcoFineLavaggioManuale)
		{
			m_bSteelcoFineLavaggioManuale = bSteelcoFineLavaggioManuale;
		}
			...
			if (listFileToParse.Count > 0)
			{
				...
				try
				{
					foreach (String sFile in listFileToParse)
					{
						...
						{
							// Sandro 04/09/2017 // BUG 834 //
							wcTemp.bSteelcoFineLavaggioManuale = m_bSteelcoFineLavaggioManuale;
		...
		public List<WasherCycle> LoadInfos(String sPath)
		{
			...
			string sOperatorStartExternalID = GetOperator(fileContent);
			...
			string sOperatorEndExternalID = GetOperator(fileContent);
			...
			foreach (string sCD in endoscopes)
			{
				...
				if (iDeviceID > 0)
				{
					...
					{ 
						...
						DesiredDestinationState = StateTransactions.GetStateId(FixedStates.Start_wash),
					};
					var endcycle = new WasherCycle 
					{
						...
						FileContent = sFileContent,
						AdditionalInfoList = listAdditionalInfo,
						DesiredDestinationState = StateTransactions.GetStateId(FixedStates.End_wash)
					};
		...
		public string GetOperator(string text)
		{
			...
			Match match = Regex.Match(text, sCleaningOperatorMatch + ".*");
			if (match.Success)
			{
				char[] delim = { '\n', '\r', /*' ', ':',*/ '.' };
				...
				sReturn = sReturn.Replace(sCleaningOperatorMatch, "");
				sReturn = sReturn.Replace("\n", "");
				sReturn = sReturn.Replace("\r", "");
				sReturn = sReturn.Replace(": ", "");
				sReturn = sReturn.Replace(".", "");
