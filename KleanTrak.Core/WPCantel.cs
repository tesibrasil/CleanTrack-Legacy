...
namespace KleanTrak.Core
{
	class WPCantel : WPBase
	{
		private enum CantelColumns
		{
			...
			UnloadOperatorID,
			UnloadOperator,
			DetergentBatchNumber,
			DisinfectantPartBSerialNumber,
			DisinfectantPartASerialNumber,
			Ignore1,
			Ignore2,
			HookupType,
			HookupSerial,
		...
		{
			List<WasherCycle> listRet = new List<WasherCycle>();

			if (washer.SerialNumber == null || washer.SerialNumber.Length == 0)
            {
                Logger.Warn(washer.ID.ToString() + " - " + washer.Code + " - " + washer.Description + " --> Washer Serial is empty!");
                return listRet;
            }
			...
			foreach (string sFile in sFiles)
			{
				...
				if (sFileLines.Length == 1)
				{
					...
					if (sFileSplitted.Length >= (int)CantelColumns.HookupGS1) // Enum.GetValues(typeof(CantelColumns)).Length)
					{
						...
						if (dtFile >= lastDate)
						{
							...
							if ((iFinalNum == 1) || (iFinalNum == 2))
							{
								Logger.Info(sFile + " --> parsing...");
								listRet.Add(ParseFile(iFinalNum, sFile, sFileLines[0], dtFile, sFileSplitted));
								DeleteFile(sFile);
							}
							else
							{
								Logger.Warn(sFile + " --> Wrong final number (" + iFinalNum.ToString() + ", expected 1 or 2)");
								DeleteFile(sFile);
							}
						}
						else
		...
		private WasherCycle ParseFile(int iFinalNum, string sFilename, string sFileContent, DateTime dtFileDatetime, string[] sFileSplitted)
		{
			...
			wcReturn.Failed = !String.Equals(sFileSplitted[(int)CantelColumns.CycleStatus], "PASS", StringComparison.OrdinalIgnoreCase);
			...
			{
				case 1: // inizio ciclo //
					wcReturn.DesiredDestinationState = StateTransactions.GetStateId(FixedStates.Start_wash);
					wcReturn.Completed = false;
					break;
				case 2: // fine ciclo //
					wcReturn.DesiredDestinationState = StateTransactions.GetStateId(FixedStates.End_wash);
					wcReturn.Completed = true;
					break;
			}
		...
		private bool DeleteFile(string sFileToDelete)
		{
			bool bDeleted = false;

#if true
			int iSlash = sFileToDelete.LastIndexOf('\\');

			string sDestFolder = sFileToDelete.Substring(0, iSlash) + "\\OLD";
			string sDestFile = sDestFolder + sFileToDelete.Substring(iSlash);
			...
			try
			{
				...
				File.Move(sFileToDelete, sDestFile);
