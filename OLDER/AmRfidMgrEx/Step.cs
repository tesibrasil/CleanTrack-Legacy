...
namespace amrfidmgrex
{
    public class Step : DBObject
    {
        ...
        {
            RFIDManager.writeLog("StartTimeoutTimer");
        ...
        {
            RFIDManager.writeLog("AnticollisionCallback");
            ...
            if (Operator.Exist(str))
            {
                ...
                RFIDManager.writeLog(this.hostName, "Cleantrack.Service", "Operator detected:" + str, (byte[])null, Logger.LogLevel.Info);
            ...
            if (Device.Exist(str))
            {
                ...
                RFIDHelper.Get().Led(RFIDHelper.LedType.Orange, true);
                RFIDManager.writeLog(this.hostName, "Cleantrack.Service", "Device detected:" + str, (byte[])null, Logger.LogLevel.Info);
        ...
        {
            RFIDManager.writeLog("AnticollisionReadOnceCallback");
            RFIDHelper.Get().Buzzer(10);
        ...
        {
            RFIDManager.writeLog("RefreshTick");
            string host = "";
            try
            {
            ...
            if (description1 != "")
                RFIDManager.writeLog(host, "Cleantrack.Service", description1, (byte[])null, Logger.LogLevel.Info);
            string description2 = Device.Refresh();
            if (description2 != "")
                RFIDManager.writeLog(host, "Cleantrack.Service", description2, (byte[])null, Logger.LogLevel.Info);
            Console.WriteLine("...Data Refreshed");
        }

        private void Complete()
        {
            RFIDManager.writeLog("complete");
        ...
        private void CompleteError()
        {
            ...
            RFIDManager.writeLog(this.hostName, "Cleantrack.Service", this.CurrentDevice.Stato.ToString() + " --> " + (object)this.Id + " : State transition not allowed", (byte[])null, Logger.LogLevel.Info);
        }
        ...
        private void CompleteTimeoutError()
        {
            ...
            RFIDManager.writeLog(this.hostName, "Cleantrack.Service", "Reading Timeout", (byte[])null, Logger.LogLevel.Info);
        }
        ...
        private void TimeoutTick(object state)
        {
            ...
            RFIDManager.writeLog(this.hostName, "Cleantrack.Service", "Timeout reading tag", (byte[])null, Logger.LogLevel.Info);
        }       

        public void Start(Types.CompleteDelegate completeCallback, int delayRefreshData, int delayTimeout, List<int> listBannedType, int cleaner, int IdExamToSave = -1)
        {
        ...
        protected void UpdateDBAsync(string deviceTag, string userTag, int cleaner, int state, int oldState, int IdExamToSave, string connectionString)
        {
            RFIDManager.writeLog(this.hostName, "Cleantrack.Service", "Updating DB: DEVICETAG:" + deviceTag + " USERTAG:" + userTag + " CLEANER:" + (object)cleaner + " OLDSTATE:" + (object)oldState + " ENDOX_EXAM:" + (object)IdExamToSave, (byte[])null, Logger.LogLevel.Info);
            RFIDManager.writeLog(this.hostName, "Cleantrack.Service", "Updating DB " + (object)DBUtilities.insertnewCycle(deviceTag, userTag, cleaner, state, oldState, IdExamToSave, connectionString), (byte[])null, Logger.LogLevel.Info);
        }
