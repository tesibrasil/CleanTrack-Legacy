...
namespace amrfidmgrex
{
  public class DBUtilities
  {
    ...
    public static RFIDCiclo GetCycleData(int iIDEsame, string sConnectionString, bool bPrevious)
    {
      int id1 = -1;
      int id2 = -1;
      int id3 = -1;
    ...
    public static RFIDCiclo GetCycleDataFromEsameDispositivo(int iIDEsame, int iIDDispositivo, string sConnectionString, bool bPrevious)
    {
    ...
    public static string getCycleAdditionalInfoFromCycle(int idCiclo, string connectionString)
    {
      ...
      if (idCiclo > 0)
      {
        ...
        try
        {
          ...
          OdbcCommand odbcCommand = new OdbcCommand();
          odbcCommand.Connection = odbcConnection;
          odbcCommand.CommandText = "SELECT DATA,DESCRIZIONE,VALORE,ERROR FROM CICLI_PLUS WHERE IDCICLI = " + (object) idCiclo + " ORDER BY ID DESC";
          reader = odbcCommand.ExecuteReader();
          while (reader.Read())
          {
            string str2;
    ...
    public static bool insertnewCycle(string deviceTag, string userTag, int cleaner, int state, int oldState, int idExamToSave, string connectionString)
    {
      ...
      if (deviceTag != null && deviceTag != "" && (userTag != null && userTag != "") && (state >= 0 && oldState >= 0))
      {
        ...
        try
        {
          ...
          string sMessage = "sp_updatedevnew(OUT, " + deviceTag + ", " + state + ", " + str + ", " + userTag + ", " + cleaner + ", " + oldState + ", " + idExamToSave + ")";
    ...
    public static int insertnewCycleForReturn(string deviceTag, string userTag, int cleaner, int state, int oldState, int idExamToSave, string connectionString)
    {
      if (oldState < 0)
        oldState = 2;
      if (deviceTag != null && deviceTag != "" && (userTag != null && userTag != "") && (state >= 0 && oldState >= 0))
