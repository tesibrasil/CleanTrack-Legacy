...
namespace It.IDnova.Fxw
{
	public class RfidUtils
	{
		...
		{
			if (s.Length == 0 || (uint)(s.Length % 2) > 0U)
				return (byte[])null;
			try
			{
		...
		{
			if (s.Length == 0 || (uint)(s.Length % 4) > 0U)
				return (ushort[])null;
			try
			{
		...
		{
			byte num = (byte)(((int)bcd & 240) >> 4);
			string str1 = num.ToString();
			num = (byte)((uint)bcd & 15U);
		...
		{
			if (((uint)valueIn2Compl14Bits & 8192U) > 0U)
			{
				//return -(short)((int)~valueIn2Compl14Bits + 1 & 16383);
