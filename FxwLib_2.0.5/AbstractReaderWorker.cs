...
namespace It.IDnova.Fxw
{
  internal abstract class AbstractReaderWorker
  {
    ...
    internal AbstractReaderWorker(
      FxwPhysicalType theRdrType,
      AbstractReaderWorker.ReceivedDataHandler callback)
    {
    ...
    internal void rawSend(RfidMsg currMsg)
    {
      ...
      else
      {
        ...
        if (this._physicalLayer.sendData(rawMsg, (int) length) == (byte) 0)
        {
          this._errorCounter = 0;
    ...
    internal void decodeRawData(byte[] dataBuffer)
    {
      ...
      int num2 = 0;
      int length;
      for (; index1 < dataBuffer.Length; index1 += length)
      {
        num1 = 0;
        num2 = 0;
        byte num3 = 0;
        while (index1 < dataBuffer.Length && dataBuffer[index1] != (byte) 42)
          ++index1;
        int num4 = index1;
        ...
        length = (int) dataBuffer[index1 + 1] + 3;
        ...
        if ((int) num3 == (int) dataBuffer[num4 + length - 1])
        {
          ...
          if (this._status == AbstractReaderWorker.Status.INVENTORY_LOOP && aMsg.CommandIdentifier == (byte) 224)
          {
